...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='scannet_shapenet55', help='scannet_shapenet55 [default: scannet_shapenet55]')
    ...
    dir = dataset_constants.scannet_desc_dataset_dict.get(FLAGS.dataset, 'none')
    if dir == 'none':
        print('Unsupported dataset type %s, exit!' % FLAGS.dataset)
        exit(1)
    param.input_dir = dir

    dict_labels = dataset_utils.scannet_label_to_int
    if param.inputWahlDescriptor: # if you want to use a different descriptor as the input to the network. In this case, it is 6x6x6x6 histogram descriptor
        descriptor_to_use = 'wahl_long_desc_run_0_'
        scannet_dataset = dataset_utils.Dataset(param.input_dir, dict_labels, descriptor_to_use)
    else:
        descriptor_to_use = 'merged_proposed_kl_bugfix5_desc_run_1_'
        scannet_dataset = dataset_utils.Dataset(param.input_dir, dict_labels, descriptor_to_use)

    ## check input labels
    N_classes = len(dict_labels)
    classes_train = scannet_dataset.train_label.shape[1]
    classes_test = scannet_dataset.test_label.shape[1]
    ...
    N_train = scannet_dataset.train_feat.shape
    N_l_train = scannet_dataset.train_label.shape
    print("Training dataset on scannet, N_train %dx%d, N_label_train %dx%d" % (N_train[0], N_train[1], N_l_train[0], N_l_train[1]))

    N_test = scannet_dataset.test_feat.shape
    N_l_test = scannet_dataset.test_label.shape
    print("Test on scannet, N_test %dx%d, N_label_test %dx%d" % (N_test[0], N_test[1], N_l_test[0], N_l_test[1]))

    fourd_cnn_proposed.train(param, scannet_dataset)